-- =============================================================================
-- TRINO SQL DDL: 6 BUSINESS REQUIREMENT VIEWS (FOR BI TOOLS, TRINO & DBT)
-- Target: Trino / Starburst | Catalog / Schema: hive.bi_gold
--
...
--   - All derived measures (achievement_rate, gap, is_overdue, etc.) are defined
--     in dbt Semantic Layer and GenBI metrics.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- BR-01: Báo cáo BSC trong năm
-- -----------------------------------------------------------------------------
CREATE OR REPLACE VIEW hive.bi_gold.vw_br01_bsc_yearly AS
SELECT
...
FROM hive.bi_silver.epm_targets t
LEFT JOIN hive.bi_silver.epm_c_assignments a
...
-- -----------------------------------------------------------------------------
CREATE OR REPLACE VIEW hive.bi_gold.vw_br02_dieu_hanh_cvct_klcd AS
SELECT
...
-- BR-03: Báo cáo Task
-- -----------------------------------------------------------------------------
CREATE OR REPLACE VIEW hive.bi_gold.vw_br03_task_report AS
SELECT
    t.task_type,
...
FROM hive.bi_silver.epm_tasks t;


-- -----------------------------------------------------------------------------
-- BR-04: Báo cáo Dự án
-- -----------------------------------------------------------------------------
CREATE OR REPLACE VIEW hive.bi_gold.vw_br04_project_report AS
SELECT
    p.name,
...
FROM hive.bi_silver.epm_projects p;


-- -----------------------------------------------------------------------------
-- BR-05: Báo cáo Lưu lượng truy cập (User Access Log)
-- -----------------------------------------------------------------------------
CREATE OR REPLACE VIEW hive.bi_gold.vw_br05_user_access_traffic AS
SELECT
    ...
    u.direct_manager,
    u.job_title
FROM hive.bi_silver.epm_user_access_log u;


-- -----------------------------------------------------------------------------
-- BR-06: Báo cáo Mục tiêu Ban Giám đốc (Mục tiêu BG)
-- -----------------------------------------------------------------------------
CREATE OR REPLACE VIEW hive.bi_gold.vw_br06_board_objectives AS
SELECT
    t.associated_objective,
    t.associated_item,
    t.target_type,
