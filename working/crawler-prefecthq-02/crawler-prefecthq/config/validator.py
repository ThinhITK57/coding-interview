...
from .api_config import PaginationType
from .job_config import ExtractionMode


class ConfigValidator:
    def validate(self, config: IngestionConfig):
    ...
    def _validate_api(
        ...
        for endpoint in api.endpoints:
            if endpoint.name in endpoint_names:
                raise ValueError(f"Duplicate endpoints: {endpoint.name}")
    ...
    def _validate_job(
        ...
        endpoint_names = {
            endpoint.name for endpoint in config.api.endpoints
        }

        if job.endpoint not in endpoint_names:
            raise ValueError(
                f"job endpoint {job.endpoint} does not exist"
            )
