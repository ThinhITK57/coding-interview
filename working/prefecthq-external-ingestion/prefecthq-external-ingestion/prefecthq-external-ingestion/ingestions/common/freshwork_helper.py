...
# =========================
# @task
def fetch_incremental_freshwork(
    resource_name,
    base_url,
    resoure_url,
    headers,
...
# @task
def fetch_resource_name_freshwork(
    resource_name, cfg, headers, max_page=300, start_page=1, **kwargs
):
    ...
    result_json_key = rc.get("result_json_key", resource_name)
    per_page = rc.get("per_page", 100)
    # crawl_mode = "static"
    ...
    records = fetch_incremental_freshwork(
        resource_name=result_json_key,
        base_url=BASE_URL,
        ...
        max_page=max_page,
        start_page=start_page, per_page=per_page
    )
    if not records:
        print("No new data")
        out_of_data = True
