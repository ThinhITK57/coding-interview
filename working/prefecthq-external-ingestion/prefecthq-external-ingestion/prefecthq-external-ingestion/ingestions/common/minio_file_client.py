...
class MinioS3Client(BaseHDFSClient):
    ...
    def read(self, hdfs_path: str) -> str:
        bucket, key = self._parse_path(hdfs_path)
    ...
    def remove(self, hdfs_path: str):
        bucket, prefix = self._parse_path(hdfs_path)
        if not bucket or not prefix:
            raise Exception(f"Invalid bucket = {bucket} , prefix={prefix}")

        objects_to_delete = [{"Key": key} for key in self.list_objects(bucket, prefix) if not key.endswith("/")]
    ...
    def remove_files(self, bucket: str, keys: list):
        objects_to_delete = [{"Key": key} for key in keys if not key.endswith("/")]
        print(f"objects_to_delete={objects_to_delete}")
    ...
    def remove_dir(self, s3_path: str):
        ...
        objects_to_delete = [{"Key": key} for key in self.list_objects(bucket, prefix)]
        print(f"objects_to_delete={objects_to_delete}")
    ...
    def delete_prefix(self, hdfs_path: str):
        bucket, prefix = self._parse_path(hdfs_path)
        if not bucket or not prefix:
            raise Exception(f"Invalid bucket = {bucket} , prefix={prefix}")
    ...
    def exists(self, hdfs_path: str) -> bool:
        ...
        try:
            ...
            # check prefix
            objs = list(self.list_objects(bucket, key))
            return len(objs) > 0
