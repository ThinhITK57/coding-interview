...
target_path = "s3a://bi-silver/hr_headcount_demand_snapshot"

# spark.sql("REFRESH TABLE hr_raw.hr_employee_headcount_logs")
from pyspark.sql.functions import col

raw_path = "s3a://vcs-raw/hr-raw/hr_employee_headcount_logs"
...
df_raw = df_raw.withColumn(
    "snapshot_date_ts",
    col("snapshot_date_ts").cast("bigint")
)

df_raw.createOrReplaceTempView("tmp_hr_employee_headcount_logs")
    ...
    SELECT
        ...
        TO_DATE(
            COALESCE(
                ...
                CASE
                    WHEN h.snapshot_date_ts IS NOT NULL AND h.snapshot_date_ts > 9999999999
                        THEN FROM_UNIXTIME(CAST(h.snapshot_date_ts / 1000 AS BIGINT))
                    WHEN h.snapshot_date_ts IS NOT NULL
                        THEN FROM_UNIXTIME(CAST(h.snapshot_date_ts AS BIGINT))
                END
            )
        ) AS snapshot_date_parsed

    FROM tmp_hr_employee_headcount_logs h
)
