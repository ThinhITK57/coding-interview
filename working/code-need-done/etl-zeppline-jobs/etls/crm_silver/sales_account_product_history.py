...
SELECT
    ...
    TRIM(p)                     AS product_name
FROM last_sales_accounts s
LATERAL VIEW explode(
    split(
        COALESCE(s.custom_field.cf_product_history, ''),
        ';'
    )
) exploded AS p
...
df.write \
    ...
    .option(
        "path",
        "/opt/datasets/crawlers/vcs_silver/crm_silver/data/sales_account_product_history"
    ) \
    .saveAsTable("crm_silver.sales_account_product_history")
