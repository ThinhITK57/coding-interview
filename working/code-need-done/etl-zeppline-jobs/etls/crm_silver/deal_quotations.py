...
df_result = df_quotation.select(
    ...
    col("quotation.created_at"),
    col("quotation.status"),
    col("quotation.tLang"),
    col("quotation.currency"),
    col("quotation.global_discount"),
    col("quotation.global_discountType").alias("global_discount_type"),
)
# spark.sql("DROP TABLE IF EXISTS crm_silver.deal_quotations")
...
df_result.write \
    ...
    .option(
        "path",
        "/opt/datasets/crawlers/vcs_silver/crm_silver/data/deal_quotations"
    ) \
    .saveAsTable("crm_silver.deal_quotations")
