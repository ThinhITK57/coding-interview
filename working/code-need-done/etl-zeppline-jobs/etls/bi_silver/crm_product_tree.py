...
tgt_table = "bi_silver.crm_product_tree"
tgt_path  = "/opt/datasets/crawlers/vcs_silver/bi_silver/data/crm_product_tree"

# spark.sql("DROP TABLE IF EXISTS bi_silver.crm_product_tree")
...
SELECT 
   ...
   CONCAT(
        ...
        COALESCE(CAST(pd.version AS STRING), ''), '__',
        COALESCE(CAST(pd.item_type AS STRING), ''), '__',
        COALESCE(CAST(pd.license AS STRING), ''), '__',
        COALESCE(CAST(pd.price_type AS STRING), ''), '__',
        COALESCE(CAST(pd.unit AS STRING), '')
    ) AS idd
...
ORDER BY product_group, product_category ASC
"""
