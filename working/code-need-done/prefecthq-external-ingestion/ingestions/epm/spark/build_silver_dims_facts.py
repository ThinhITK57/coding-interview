"""
Spark ETL Pipeline: Silver Base Entities -> Silver Dimensions & Facts (Kimball Constellation Model)
Author: Data Engineering Team
Target: Python 3.7.1 | Apache Spark 2.3.2 | Java 8

Architecture (Option B):
  Bronze -> Silver Base Entities -> Silver Dims & Facts -> Gold Business Marts

Builds:
  - 7 Conformed Dimensions:
      1. dim_date (Spine 2020-2030)
      2. dim_department (Conformed cross-table)
      3. dim_resource (Conformed cross-table users/resources)
      4. dim_project (SCD Type 2: PM, Dept, Status tracking)
      5. dim_task (WBS Structure & Jira integration)
      6. dim_objective (BSC Strategic Hierarchy)
      7. dim_assignment (PGNV Hierarchy & Weight)
  - 5 Periodic Snapshot Facts:
      1. fact_target_bsc_snapshot (BR 1 & BR 6)
      2. fact_cvct_execution_snapshot (BR 2)
      3. fact_task_execution_snapshot (BR 3)
      4. fact_project_progress_snapshot (BR 4)
      5. fact_epm_user_access (BR 5)
"""
...
def build_dim_date(spark, start_year=2020, end_year=2030):
    """Generates a 10-year calendar spine dimension."""
...
def build_dim_department(projects_df, tasks_df, targets_df, objectives_df, assignments_df):
    """Conforms unique departments across all entities."""
    ...
    for df, col_name in [
        ...
        if col_name in df.columns:
            depts.append(df.select(F.col(col_name).cast(StringType()).alias("department_name")).filter("department_name is not null"))
    ...
    # Add surrogate key using monotonically_increasing_id or hash
...
def build_dim_resource(projects_df, tasks_df, targets_df, assignments_df, access_log_df):
    """Conforms unique users/resources across all entities."""
...
def build_dim_project(projects_df):
    """Builds dim_project with SCD Type 2 tracking support."""
...
def build_dim_task(tasks_df):
    """Builds dim_task representing the WBS structure."""
...
# ==============================================================================
# 2. PERIODIC SNAPSHOT FACT BUILDERS
# ==============================================================================

def build_fact_target_bsc_snapshot(targets_df, assignments_df):
    """Builds fact_target_bsc_snapshot for BR 1 & BR 6."""
    logger.info("Building fact_target_bsc_snapshot...")
    t = targets_df
...
def build_fact_cvct_execution_snapshot(targets_df):
    ...
    fact = t.select(
        ...
        t["resources_and_placeholders_count"].cast(StringType()).alias("resources") if "resources_and_placeholders_count" in t.columns else F.lit(None).alias("resources"),
        t["name"].cast(StringType()).alias("name"),
...
def build_fact_epm_user_access(access_log_df):
    ...
    fact = u.select(
        ...
        u["first_name"].cast(StringType()).alias("first_name") if "first_name" in u.columns else F.lit(None).alias("first_name"),
        u["last_name"].cast(StringType()).alias("last_name") if "last_name" in u.columns else F.lit(None).alias("last_name"),
        u["groups"].cast(StringType()).alias("groups") if "groups" in u.columns else F.lit(None).alias("groups"),
        u["direct_manager"].cast(StringType()).alias("direct_manager") if "direct_manager" in u.columns else F.lit(None).alias("direct_manager"),
        u["job_title"].cast(StringType()).alias("job_title") if "job_title" in u.columns else F.lit(None).alias("job_title")
    )
    return fact
...
def main():
    parser = argparse.ArgumentParser(description="Build Silver Dimensions and Facts (Option B Kimball Model)")
