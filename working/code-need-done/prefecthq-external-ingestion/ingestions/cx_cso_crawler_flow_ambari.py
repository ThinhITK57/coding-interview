...
@flow(log_prints=True, name="[Hourly]-Ambari CX CSO APIs - Data Lake")
def run_scrapping_cso_crawler():
    ...
    filename = os.path.join(RESOURCE_BASE_DIR, "resources-cx-cso-ambari.json")
...
@flow(log_prints=True, name="[Adhoc]-Ambari CX CSO APIs - Data Lake Single Table")
...
if __name__ == "__main__":
    d1 = run_scrapping_cso_crawler.to_deployment(
        name="[Hourly]-Ambari CX CSO APIs - Data Lake",
        cron="5 * * * *",
        tags=["production", "CSO", "ingestion"],
    )
    
    d3 = run_adhoc_scrapping_cso_crawler_by_tablename.to_deployment(
        name="[Adhoc]-Ambari CX CSO APIs - Data Lake Single Tablee",
