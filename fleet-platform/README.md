...
- Enables complete before/after captures for SCD Type 2 and Redis cache invalidations.

### 2. Sub-Millisecond Station Matching & Zero-Overbooking Concurrency
- Station Matching via Redis `GEOSEARCH` in < 1ms.
- Atomic slot reservation via Redis Lua Script guaranteeing zero overbooking.

### 3. PySpark Structured Streaming to HDFS Data Lake
- Reads JSON streams from Kafka topics and writes partitioned Parquet files (`year=YYYY/month=MM/day=DD`) with checkpointing.

### 4. Data Warehouse Star Schema & SCD Type 2 Customer Dimension
- Star schema with SCD Type 2 historical dimensions and scheduled Airflow DAGs.

### 5. Push-Based Serving Layer (Zero-Polling)
- Event-driven push model via Redis Pub/Sub down to WebSocket client.
...
│   ├── LAZYVIM-DATA-ENGINEERING-SETUP.md
│   ├── MINIKUBE-COMPLETE-GUIDE.md
│   └── VMWARE-DEV-ENVIRONMENT-SETUP.md
│
